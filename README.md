...
**下面是我的配置,电脑配置低就两个Node节点(建议最低4c8G,生产线上都是100G以上的物理机部署的),master节点建议也跑kubelet但是请打上污点,聚合路由有空测试试试**
 ...
 * 07-master.yml: 管理组件,检测apiserver端口那个`curl -sk https://master[0]:6443/healthz`的uri模块写法不知道是不是没调对经常报错,测试apiserver端口这步会出现带有`...ignoring`报错请忽略,执行完运行下`kubectl get cs`有输出就不用管
